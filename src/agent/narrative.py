...
The graph deliberately owns only three generated narrative fields.  Overall
conclusion and risk points are read-only retrieval tasks: they provide
evidence context but are never generated by the model or merged from its
response.  All other deterministic prediction fields are copied from the
baseline.
"""
...
except ImportError:  # deterministic/injected tests can use the direct runner
...
# These tasks are independently retrieved.  The first two are deterministic
# summary fields and remain locked; they are included only to make their
# evidence preparation auditable before any OfficialAnswerComposer hand-off.
...
_MERGE_RETRIEVAL_TASK_FIELDS = ENHANCED_FIELDS + (
    "overall_conclusion",
    "risk_points",
)
_CONTEXT_KEYS = {
...
_RETRIEVAL_SOURCE_QUOTA = {"knowledge_card": 2, "gold_label": 1}
_VISIBLE_SOURCE_QUOTA = {"domain_knowledge": 2, "label_example": 1}
...
_FACILITY_SUFFIXES = (
    ...
    ("隧道", "tunnel", "隧道"), ("桥式通道", "bridge", "桥式通道"),
    ("大桥", "bridge", "桥梁"), ("中桥", "bridge", "桥梁"),
...
def _limit_task_hits(hits: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw_hit in hits:
        ...
        if bucket not in _RETRIEVAL_SOURCE_QUOTA:
            # Narrative retrieval is deliberately limited to domain knowledge
            # and label examples. Do not backfill arbitrary report evidence
            # when a retriever ignores source_quota.
            if bucket == "report_evidence":
                continue
            unknown.append(hit)
            continue
...
def _retrieval_task_records(
    ...
    for task in RETRIEVAL_TASK_FIELDS:
...
def _compact_retrieval_by_task(value: Any, task_queries: Mapping[str, str] | None = None) -> dict[str, Any]:
    """Keep each task's query, quota, counts, and bounded evidence anchors."""

    queries = task_queries if isinstance(task_queries, Mapping) else {}
    if not value and not queries:
        return {}
    ...
    for task in RETRIEVAL_TASK_FIELDS:
        ...
        hits = _task_hits(raw)
        compact[task] = {
            "query": str(raw_map.get("query", queries.get(task, ""))),
            "source_quota": dict(raw_map.get("source_quota", _VISIBLE_SOURCE_QUOTA)),
...
def _filter_current_sample_retrieval(
    hits: Sequence[Mapping[str, Any]], sample_id: Any
) -> list[dict[str, Any]]:
    """Drop cross-sample report evidence before it can reach a prompt or trace."""

    current = str(sample_id or "").strip()
    result: list[dict[str, Any]] = []
    for raw in hits:
        if not isinstance(raw, Mapping):
            continue
        hit = dict(raw)
        if _retrieval_source_bucket(hit) == "report_evidence":
            hit_sample = str(hit.get("sample_id", "") or "").strip()
            if not current or not hit_sample or hit_sample != current:
                continue
        result.append(hit)
    return result
...
def _retrieve_knowledge(state: NarrativeState, retriever: Any) -> dict[str, Any]:
    ...
    for task in RETRIEVAL_TASK_FIELDS:
        ...
        if results is None:
            results = []
        task_hits[task] = _filter_current_sample_retrieval(
            [dict(item) for item in _jsonable(results) if isinstance(item, Mapping)],
            state.get("sample_id", ""),
        )
...
def _clean_generated_paragraph(value: Any) -> str:
    text = str(value or "").strip()
    return re.sub(r"^(?:[-*•]|\d+[.、)]|[（(]?[一二三四1234][）)])\s*", "", text).strip()


def _official_paragraph_slot(text: str) -> int | None:
    stripped = _clean_generated_paragraph(text)
    for index, expected in enumerate(_OFFICIAL_PARAGRAPH_PREFIXES):
...
def _repair_detailed_paragraphs(value: Any, baseline: Mapping[str, Any]) -> Any:
    """Repair model formatting without inventing report facts.

    Qwen/DeepSeek occasionally return one long string or omit one official
    paragraph.  We may split by official prefixes and fill only a missing
    paragraph from the deterministic baseline.  We never synthesize a new
    paragraph here.
    """
    ...
    if isinstance(value, str):
        ...
        parts = [
            part.strip(" \t\r\n;；")
            for part in re.split(r"(?=(?:经综合评定|本次为|本次报告|目前|综上))", text)
            if part.strip(" \t\r\n;；")
        ]
        raw_items = parts if len(parts) > 1 else [text]
    elif isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        raw_items = list(value)
    else:
        return value

    generated = [_clean_generated_paragraph(item) for item in raw_items if isinstance(item, str) and item.strip()]
    if len(generated) == 4 and all(_official_paragraph_slot(item) == index for index, item in enumerate(generated)):
        return generated

    slots: list[str | None] = [None, None, None, None]
    for item in generated:
        slot = _official_paragraph_slot(item)
        if slot is not None and slots[slot] is None:
            slots[slot] = item

    baseline_value = baseline.get("detailed_conclusion", [])
    if isinstance(baseline_value, Sequence) and not isinstance(baseline_value, (str, bytes, bytearray)):
        for item in baseline_value:
            if not isinstance(item, str):
                continue
            cleaned = _clean_generated_paragraph(item)
            slot = _official_paragraph_slot(cleaned)
            if slot is not None and slots[slot] is None:
                slots[slot] = cleaned
...
def _normalise_evidence_items(value: Any) -> Any:
    if isinstance(value, str):
        value = [value]
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes, bytearray)):
        return value
    ...
    for item in value:
        if isinstance(item, str):
            normalised.append({"text": item.strip(), "evidence_ids": []})
            continue
        if not isinstance(item, Mapping):
            normalised.append(item)  # validator will reject this item safely
            continue
        text = item.get("text")
        if not isinstance(text, str):
            text = next(
                (item.get(key) for key in ("content", "description", "conclusion") if isinstance(item.get(key), str)),
                text,
            )
        evidence_ids = item.get("evidence_ids")
        if evidence_ids is None:
            evidence_ids = item.get("evidence_id", item.get("source_ids", item.get("citations", [])))
        if isinstance(evidence_ids, (str, int)) and not isinstance(evidence_ids, bool):
            evidence_ids = [evidence_ids]
        normalised.append({"text": text, "evidence_ids": evidence_ids})
    return normalised


def _normalise_generated_sections(payload: Mapping[str, Any], baseline: Mapping[str, Any]) -> dict[str, Any]:
    """Keep only merge-owned fields and repair harmless model shape drift."""
    ...
    for field in ENHANCED_FIELDS:
        ...
        if field == "detailed_conclusion":
            value = _repair_detailed_paragraphs(value, baseline)
        else:
            value = _normalise_evidence_items(value)
        generated[field] = _jsonable(value)
    return generated
...
def _generate_narrative(state: NarrativeState, client: Any) -> dict[str, Any]:
    ...
    try:
        ...
        baseline = state.get("baseline_prediction", {})
        baseline = baseline if isinstance(baseline, Mapping) else {}
        generated = _normalise_generated_sections(payload, baseline)
...
def _safety_priority(state: NarrativeState) -> tuple[set[str], str, str]:
    ...
    # Safety conclusions must be grounded in the current report. Domain
    # knowledge and label examples may guide wording but cannot substitute for
    # a current-report safety/defect fact.
    return set(), "current report safety or defect evidence", ""
...
def _facility_semantic_errors_by_field(
    ...
    type_nouns = {noun for (_, ftype, noun) in _FACILITY_SUFFIXES if ftype == facility_type}
    allowed_nouns = {facility_noun, "设施", "通道"} | type_nouns
    for field in ENHANCED_FIELDS:
    ...
    if isinstance(safety_values, list):
        if safety_values and not priority_ids:
            add("safety_impact", "safety_impact requires current report safety or defect evidence")
        for index, item in enumerate(safety_values):
            ...
            if priority_ids and not item_ids.intersection(priority_ids):
                add("safety_impact", f"safety_impact[{index}] must cite {priority_label}")
...
def _canonical_sections(
    ...
    *,
    ...
    valid_fields: Sequence[str] = ENHANCED_FIELDS,
) -> dict[str, Any]:
    """Canonicalize valid fields and preserve the baseline per failed field."""
...
def _finalize(state: NarrativeState) -> dict[str, Any]:
    ...
    field_results = {
        field: ("enhanced" if field in valid_fields else "fallback")
        for field in ENHANCED_FIELDS
    }
    return {
        "enhanced_prediction": enhanced,
        "generated_sections": canonical,
        "used_fallback": bool(fallback_fields),
        "field_results": field_results,
        "validation_errors": list(state.get("validation_errors", [])),
        "field_fallbacks": fallback_fields,
    }
...
def build_narrative_graph(
    ...
    if StateGraph is None:
        class _DirectGraph:
            ...
            def invoke(self, initial: NarrativeState) -> NarrativeState:
                state: NarrativeState = dict(initial)
                state.update(_prepare_context(state, max_retries))
                state.update(_retrieve_knowledge(state, retriever))
                while True:
                    state.update(_generate_narrative(state, client))
                    state.update(_validate_output(state))
                    if state.get("validation_passed"):
                        break
                    if int(state.get("retry_count", 0)) >= int(state.get("max_retries", 0)):
                        break
